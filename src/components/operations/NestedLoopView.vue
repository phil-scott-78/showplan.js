<template>
  <div>
    <div v-if="nestedLoop.Predicate !== undefined" class="content">
      <h4>Predicate</h4>
      <div><sql-string :sql="nestedLoop.Predicate.ScalarOperator.ScalarString"></sql-string></div>
    </div>

    <div v-if="nestedLoop.PassThru !== undefined" class="content">
      <h4>PassThru</h4>
      <div><sql-string :sql="nestedLoop.PassThru.ScalarOperator.ScalarString"></sql-string></div>
    </div>

    <div v-if="nestedLoop.OuterReferences !== undefined && nestedLoop.OuterReferences.length > 0" class="content">
      <h4>Outer References</h4>
      <ul class="small">
        <li v-for="(column, index) in nestedLoop.OuterReferences" :key="index"><sql-string :sql="column.toString()"></sql-string></li>
      </ul>
    </div>
  </div>
...
import { RelOp, NestedLoops } from '@/parser/showplan';
