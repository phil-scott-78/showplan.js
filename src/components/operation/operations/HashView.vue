<template>
    <div>
        <div
            v-if="hash.HashKeysBuild !== undefined && hash.HashKeysBuild.length > 0"
            class="content"
        >
            <h4>Hash Keys Build</h4>
            <ul class="small">
                <li
                    v-for="(column, index) in hash.HashKeysBuild"
                    :key="index"
                >
                    <sql-string
                        :sql="column.toString()"
        ...
        <div
            v-if="hash.BuildResidual !== undefined"
            class="content"
        >
            <h4>Build Residual</h4>
            <div class="small">
                <sql-string
                    :sql="hash.BuildResidual.ScalarOperator.ScalarString"
                    :expanded-columns="expandedChildColumns"
                />
            </div>
        </div>
        <div
            v-if="hash.HashKeysProbe !== undefined && hash.HashKeysProbe.length > 0"
            class="content"
        >
            <h4>Hash Keys Probe</h4>
            <ul class="small">
                <li
                    v-for="(column, index) in hash.HashKeysProbe"
                    :key="index"
                >
                    <sql-string
                        :sql="column.toString()"
        ...
        <div
            v-if="hash.ProbeResidual !== undefined"
            class="content"
        >
            <h4>Probe Residual</h4>
            <div class="small">
                <sql-string
                    :sql="hash.ProbeResidual.ScalarOperator.ScalarString"
                    :expanded-columns="expandedChildColumns"
                />
            </div>
        </div>
        <div
            v-if="hash.BitmapCreator !== undefined"
            class="content"
        >
            Bitmap Creator: <strong>{{ hash.BitmapCreator }} </strong>
        </div>
        <defined-values
            v-if="hash.DefinedValues !== undefined && hash.DefinedValues.length > 0"
            :defined-values="hash.DefinedValues"
            :expanded-columns="expandedChildColumns"
        />
    </div>
</template>
